...
Keras 3 picks a backend at import time and defaults to TensorFlow, which NTB
does not depend on. The test suite sets `KERAS_BACKEND=torch` for you; set it
yourself if you run Keras outside pytest.
...
3. The cross-backend numeric parity test is generated from your declaration — you
   do not write it. If the op has weights, declare their Keras layout
   (`WeightSpec`) so the harness can give both backends the same model.
4. If a backend cannot express what the op means for some attribute, add a
   `guard` rather than an approximation. A padded max pool refuses in Keras
   because zero padding and -inf padding are different models.
